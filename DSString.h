...
public:
     ...
     * could be const, but then we would have to implement a const iterator in
     * DSVector.
     **/
    friend std::ostream &operator<<(std::ostream &, const DSString &);
    friend std::ifstream &operator>>(std::ifstream &, DSString &);
    ...
    int length() const;
    char getAtChar(size_t start) const;
    DSString getAtString(size_t start) const;
    DSString apndCharToString(char newChar) const;
};
